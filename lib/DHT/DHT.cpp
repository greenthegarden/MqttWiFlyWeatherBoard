...
// 0.1.14 replace digital read with faster (~3x) code => more robust low MHz
// machines.
//
...
int8_t DHT::read11(uint8_t pin) {
  // READ VALUES
  int8_t result =
      _readSensor(pin, DHTLIB_DHT11_WAKEUP, DHTLIB_DHT11_LEADING_ZEROS);
...
int8_t DHT::_readSensor(uint8_t pin, uint8_t wakeupDelay,
  ...
  for (uint8_t i = 40; i != 0;) {
    ...
    if (state == LOW && pstate != LOW) {
      if (i >
          leadingZeroBits) // DHT22 first 6 bits are all zero !!   DHT11 only 1
      {
